function SevnJS(){const license="copyrights Prateek Raj Gautam, soon to be released under Apache 2.0";const version=`v0.9.3`;const grab=(parentidstr)=>{var match=parentidstr.match(/^([^\[]*)\[*(\d*)\]*$/mi);var parentid=match[1];var parentElement=null;try{if(parentid instanceof Object==true){parentElement=parentid}
...
if(Array.isArray(childhtml)){childhtml.forEach((child,index)=>{if(typeof child=='string'){T.innerHTML+=child;}else if(typeof child!='string'){if(child.outerHTML!=undefined){T.innerHTML+=child.outerHTML;}
